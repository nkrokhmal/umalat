...
class Validator(ClassValidator):
    ...
    def validate__boiling_group__boiling_group(b1, b2):
        for line_num in range(3):
            if line_num not in b1.props["line_nums"] or line_num not in b2.props["line_nums"]:
                # skip if no common line number is used
                continue

            for boiling1 in [
                boiling
                for i, boiling in enumerate(b1["boiling_sequence"]["boiling", True])
                if b1.props["line_nums"][i] == line_num
            ]:
                for boiling2 in [
                    boiling
                    for i, boiling in enumerate(b2["boiling_sequence"]["boiling", True])
                    if b2.props["line_nums"][i] == line_num
                ]:
    ...
    def validate__boiling_group__container_cleanings(b1, b2):
        ...
        validate_disjoint_by_axis(b1["analysis_group", True][-1]["analysis"], b2["container_cleaning_2"])
        validate_disjoint_by_axis(b1["analysis_group", True][-1]["pumping"], b2["container_cleaning_3"])
    ...
    with code("make_boilings"):
        for bg_prev, bg in iter_pairs(boiling_groups, method="any_prefix"):
            ...
            line_nums_props = [
                ...
                [1, 2, 0] * 100,
                [2, 0, 1] * 100,
            ]  # allow maximum up to 300 tanks per boiling. Now used 4 maximum (2022.08.29)
            if first_tank:
