...
def read_boiling_plan(wb_obj, saturate=True, first_batch_ids_by_type=None):
    """
    :param wb_obj: str or openpyxl.Workbook
    :return: pd.DataFrame(columns=['id', 'boiling', 'sku', 'kg'])
    """
    first_batch_ids_by_type = first_batch_ids_by_type or {"ricotta": 1}
    ...
    for i in range(2, 200):
        ...
        if ws.cell(i, NAME_COLUMN).value == "-":
            ...
            kg_per_tank = float(int(ws.cell(i, OUTPUT_COLUMN).value) / int(number_of_tanks_in_boiling))
    ...
    df = update_absolute_batch_id(df, first_batch_ids_by_type=first_batch_ids_by_type)
    return df


def test():
    wb = load_workbook(str(get_repo_path() / "app/data/tests/ricotta/boiling.xlsx"))
    df = read_boiling_plan(wb)
    print(df)
