...
def make_mascorpone_boiling(boiling_group_df, **props):
    ...
    with m.row("packing_process", push_func=add_push, x=(packing_process_start, 0)):
        ...
        with m.row("packing_group"):
            prev_p = None
            prev_packing = None
            for ind, grp in df_to_ordered_tree(boiling_group_df, column="boiling_key", recursive=False):
                bt = get_boiling_technology_from_boiling_model(grp.iloc[0]["boiling"])

                ingredient = m.row("ingredient", push_func=add_push, size=bt.ingredient_time // 5, x=start_from).block
                ...
                if not prev_p:
                    ...
                    p = m.row(
                        "P",
                        push_func=add_push,
                        x=max([prev_p.y[0], prev_packing.y[0] - 1, ingredient.y[0]])
                        - m.root["packing_process"]["N", True][-1].y[0],
                        size=2,
                    ).block

                packing_time = sum([row["kg"] / row["sku"].packing_speed * 60 for i, row in grp.iterrows()])
                packing_time = int(custom_round(packing_time, 5, "ceil"))

                packing = m.row(
                    "packing", push_func=add_push, x=p.props.relative_props["x"][0] + 1, size=packing_time // 5
                ).block
...
def make_mascarpone_boiling_group(boiling_group_dfs):
    ...
    class Validator(ClassValidator):
        ...
        def validate__boiling__boiling(b1, b2):
            ...
            assert (
                b1["packing_process"]["packing_group"]["P", True][-1].x[0] <= b2["boiling_process"]["pumping_off"].x[0]
            )

    m = BlockMaker("mascarpone_boiling_group", boiling_group_dfs=boiling_group_dfs)

    mascarpone_boilings = [
        make_mascorpone_boiling(boiling_group_df, n=i) for i, boiling_group_df in enumerate(boiling_group_dfs)
    ]
