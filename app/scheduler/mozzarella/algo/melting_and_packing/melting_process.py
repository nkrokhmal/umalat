...
def fill_configurations(maker, mpps, boiling_model):
    ...
    for mpp1, mpp2 in utils.SimpleIterator(mpps).iter_sequences(2):
...
def make_melting_and_packing_from_mpps(boiling_model, mpps):
    maker, make = utils.init_block_maker("melting_and_packing", axis=0)

    class_validator = utils.ClassValidator(window=3)

    def validate(b1, b2):
        utils.validate_disjoint_by_axis(b1["melting_process"], b2["melting_process"])
    ...
    def validate(b1, b2):
        ...
        for packing in packings:
            utils.validate_disjoint_by_axis(packing, b2)
    ...
    for c in blocks:
        if c.props["cls"] == "packing_configuration":
            utils.push(
                maker.root,
                c,
                push_func=lambda parent, block: utils.AxisPusher(start_from="last_end")(
            ...
            utils.push(
                maker.root,
                c,
                push_func=lambda parent, block: utils.AxisPusher(start_from=start_from)(
                    parent, block, validator=class_validator
                ),
            )
            ...
            if blocks:
                ...
                with make(
                    ...
                    for block in blocks:
                        ...
                        make(block, push_func=utils.add_push)
    return maker.root
