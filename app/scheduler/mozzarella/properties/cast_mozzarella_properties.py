...
def cast_mozzarella_properties(schedule=None):
    ...
    skus = sum(
        [list(b.props["boiling_group_df"]["sku"]) for b in schedule["master"]["boiling", True]],
        [],
    )
    ...
    with code("multihead"):
        multihead_packings = list(
            schedule.iter(
                cls="packing", boiling_group_df=lambda df: df["sku"].iloc[0].packers[0].name == "Мультиголова"
            )
        )
    ...
    props.short_cleaning_times = [
        ...
        for cleaning in schedule["master"]["cleaning", True]
        if cleaning.props["cleaning_type"] == "short"
    ]
    props.full_cleaning_times = [
        cast_human_time(cleaning.x[0])
        for cleaning in schedule["master"]["cleaning", True]
        if cleaning.props["cleaning_type"] == "full"
    ]

    # - Meltings

    salt_boilings = [
        b for b in schedule["master"]["boiling", True] if b.props["boiling_model"].line.name == "Пицца чиз"
    ]
    if salt_boilings:
        props.salt_melting_start_time = cast_human_time(
            salt_boilings[0]["melting_and_packing"]["melting"]["meltings"].x[0]
        )
    ...
    lines_df.loc["water", "boilings"] = [
        b for b in schedule["master"]["boiling", True] if b.props["boiling_model"].line.name == "Моцарелла в воде"
    ]
    lines_df.loc["salt", "boilings"] = [
        b for b in schedule["master"]["boiling", True] if b.props["boiling_model"].line.name == "Пицца чиз"
    ]
    lines_df["melting_end"] = lines_df["boilings"].apply(
        lambda boilings: None if not boilings else boilings[-1]["melting_and_packing"]["melting"].y[0]
    )
