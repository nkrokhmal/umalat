...
def fill_boilings():
    ...
    for b in b_data:
        ...
        bt_id = [x for x in bts if (x.heating_time == b['Нагрев']) &
                 (x.delay_time == b['Выдержка']) &
                 (x.protein_harvest_time == b['Сбор белка']) &
                 (x.abandon_time == b['Заборс']) &
                 (x.pumping_out_time == b['Слив']) &
                 (x.name == RicottaBoilingTechnology.create_name(
                     line=line_name, percent=b['Процент'], flavoring_agent=b['Вкусовая добавка']))][0].id
