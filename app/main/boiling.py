...
def get_boiling():
    ...
    return render_template(
        "get_boiling.html", boilings=boilings, endpoints=".get_boiling"
    )
...
def edit_boiling(boiling_id):
    ...
    with db.session.no_autoflush:
        ...
        if form.validate_on_submit() and boiling is not None:
            ...
            boiling_technology = (
                db.session.query(BoilingTechnology)
                .filter_by(BoilingTechnology.pouring_time == form.pouring_time.data)
                .filter_by(
                    BoilingTechnology.soldification_time == form.soldification_time.data
                )
                .filter_by(BoilingTechnology.cutting_time == form.cutting_time.data)
                .filter_by(
                    BoilingTechnology.pouring_off_time == form.pouring_off_time.data
                )
            ...
            if form.line.data != -1:
                boiling.line = [
                    x for x in form.lines if x.name == get_choice_data(form.line)
                ][0]
