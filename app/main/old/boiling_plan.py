...
def boiling_plan_full():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        df = df[
            df["plan"].apply(
                lambda x: type(x) == int or type(x) == float or x.isnumeric()
            )
        ]
        path = "{}/{}.csv".format(
            current_app.config["STATS_FOLDER"], os.path.splitext(file.filename)[0]
        )
        ...
        new_file_name = draw_constructor_template(
            full_plan, file.filename, template_wb, df_extra_packing
        )
        if new_file_name != file_path:
            os.remove(file_path)
        return render_template(
            "boiling_plan_full.html", form=form, file_name=new_file_name
        )
    file_name = None
