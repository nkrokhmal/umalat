...
from app.main.forms import SkuPlanForm
...
def sku_plan():
    try:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            group_items = [
                {"BoilingId": x.made_from_boilings[0].id} for x in skus.copy()
            ]
            group_items = [
                dict(x) for x in set(frozenset(d.items()) for d in group_items)
            ]
            ...
            for item in data:
                ...
                if sku is not None and len(sku) > 0:
                    full_list.append({"SKU": sku[0], "Request": item[2]})
            ...
            for group_item in group_items:
                group_sku = [
                    x
                    for x in full_list
                    if x["SKU"].made_from_boilings[0].id == group_item["BoilingId"]
                ]
                if len(group_sku) > 0:
                    output_weight = group_sku[0]["SKU"].line.output_ton
                    request_weight = sum(
                        [x["Request"] for x in group_sku if x["Request"] < 0]
                    )
                    result_list.append(
                        {
                            "GroupSKU": group_sku,
                            ...
                            "IsLactose": group_sku[0]["SKU"]
                            .made_from_boilings[0]
                            .is_lactose,
                        }
                    )
