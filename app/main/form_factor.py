...
def add_form_factor():
    ...
    if form.validate_on_submit():
        form_factor = FormFactor(relative_weight=form.relative_weight.data)

        cooling_technology = (
            db.session.query(CoolingTechnology)
            .filter_by(
                or_(
                    form.first_cooling_time.data is None,
                    CoolingTechnology.first_cooling_time
                    == form.first_cooling_time.data,
                )
            )
            .filter_by(
                or_(
                    form.second_cooling_time.data is None,
                    CoolingTechnology.second_cooling_time
                    == form.second_cooling_time.data,
                )
            )
            .filter_by(
                or_(
                    form.salting_time.data is None,
                    CoolingTechnology.salting_time == form.ssalting_time.data,
                )
            )
            .first()
        )
        ...
        try:
            ...
            flash(
                "Exception occurred in add_boiling_form_factor request. Error: {}.".format(
                    e
                ),
                "error",
            )
...
def get_form_factor():
    ...
    return render_template(
        "get_form_factor.html",
        form_factors=form_factors,
        paginations=pagination,
        endopoints=".get_form_factor",
    )
