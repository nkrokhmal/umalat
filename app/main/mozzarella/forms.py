...
from ...models import Packer, MozzarellaBoiling, MozzarellaLine, PackType, FormFactor, MozzarellaSKU, \
    MozzarellaBoilingTechnology, Group, BatchNumber, MozzarellaFormFactor
from ... import db
import datetime
...
>>>>>>> dev_nk:app/main/mozzarella/forms.py
...
class FormFactorForm(FlaskForm):
    ...
    first_cooling_time = IntegerField(
        "Введите первое время охлаждения", validators=[Optional()]
    )
    second_cooling_time = IntegerField(
        "Введите второе время охлаждения", validators=[Optional()]
    )
