...
def get_washer():
    washers = db.session.query(Washer).join(Department).filter(Department.name == DepartmentName.MOZZ).all()
    return render_template('mozzarella/get_washer.html', washers=washers, endpoints='.get_washer')
...
def edit_washer(washer_id):
    ...
    return render_template('mozzarella/edit_washer.html', form=form, washer_id=washer.id)
