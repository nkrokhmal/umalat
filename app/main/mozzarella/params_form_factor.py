...
from ...models import MozzarellaFormFactor
...
def get_form_factor():
    ...
    water_form_factors = sorted([ff for ff in form_factors if (ff.name != 'Масса') and ('Терка' not in ff.name) and (ff.line.name == LineName.WATER)],
                                key=lambda ff: ff.name)
    salt_form_factors = sorted([ff for ff in form_factors if (ff.name != 'Масса') and ('Терка' not in ff.name) and (ff.line.name == LineName.SALT)],
                               key=lambda ff: ff.name)
...
def edit_form_factor(form_factor_id):
    ...
    return render_template('mozzarella/edit_form_factor.html', form=form, form_factor_id=form_factor.id)
