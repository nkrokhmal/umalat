...
def download_contour_washer_history(page):
    ...
    for date_dir in date_dirs:
        if is_date(date_dir):
            cw_dir = os.path.join(
                flask.current_app.config["DYNAMIC_DIR"], date_dir, flask.current_app.config["APPROVED_FOLDER"]
            )
    ...
    return flask.render_template(
        "history/download_contour_washers.html",
...
def download_boiling_plans(page):
    ...
    for date_dir in date_dirs:
        if is_date(date_dir):
            bp_dir = os.path.join(
                flask.current_app.config["DYNAMIC_DIR"], date_dir, flask.current_app.config["BOILING_PLAN_FOLDER"]
            )
    ...
    return flask.render_template(
        "history/download_boiling_plans.html",
        pagination=pagination,
        data=bp_result,
    )
...
def download_schedules(page):
    ...
    for date_dir in date_dirs:
        if is_date(date_dir):
            schedule_dir = os.path.join(
                flask.current_app.config["DYNAMIC_DIR"], date_dir, flask.current_app.config["SCHEDULE_FOLDER"]
            )
                ...
                if schedules_filenames:
                    ...
                    for schedules_filename in schedules_filenames:
                        ...
                        if department not in schedules_metadata[date_dir].keys():
                            ...
                            schedules_metadata[date_dir][department]["is_approved"] = is_approved(
                                schedules_filename, date_dir
                            )
    ...
    return flask.render_template(
        "history/download_schedules.html",
        pagination=pagination,
        data=schedules_result,
    )
...
def download_schedule_tasks(page):
    ...
    for date_dir in date_dirs:
        if is_date(date_dir):
            schedule_dir = os.path.join(
                flask.current_app.config["DYNAMIC_DIR"], date_dir, flask.current_app.config["TASK_FOLDER"]
            )
            if os.path.exists(schedule_dir):
                schedules_filenames = os.listdir(schedule_dir)
                if schedules_filenames:
    ...
    return flask.render_template(
        "history/download_schedule_tasks.html",
        pagination=pagination,
        data=schedules_result,
    )
