...
def make_melting_and_packing_from_mpps(boiling_model, mpps):
    ...
    for c in blocks:
        push(
            maker.root,
            c,
            push_func=lambda parent, block: AxisPusher(start_from="last_beg")(
    ...
    with make("melting"):
        ...
        with make("meltings", x=(serving.size[0], 0), push_func=add_push):
            for i, block in enumerate(listify(mp["melting_and_packing_process"])):
                make(
                    "melting_process",
                    size=(block["melting_process"].size[0], 0),
    ...
    for packing_team_id in range(1, 3):
        all_packing_processes = list(
            listify(mp["melting_and_packing_process"])[0].iter(
                cls="process", packing_team_id=packing_team_id
            )
        )
        ...
        if all_packing_processes:
            for group_key in ["packing", "collecting"]:
                with make(
                    group_key,
                    x=(
                        all_packing_processes[0].x[0]
                        + maker.root["melting"]["meltings"].x[0],
                        0,
                    ),
                    ...
                    for block in mp.children:
                        if block.props["cls"] == "packing_configuration":
                            ...
                            packings = list(
                                block.iter(
                                    cls=group_key, packing_team_id=packing_team_id
                                )
                            )
                            if not packings:
                                continue
