...
def draw_boiling_plan(df, df_extra, wb):
    ...
    for sheet_name in ['Соль', 'Вода']:
        ...
        for v in values:
            ...
            if sheet_name == 'Вода':
                formula = '=IF({1}{0}="-", "", 1 + SUM(INDIRECT(ADDRESS(2,COLUMN({2}{0})) & ":" & ADDRESS(ROW(),COLUMN({2}{0})))))'\
                    .format(cur_row, COLUMNS['delimiter'].col_name, COLUMNS['delimiter_int'].col_name)
            else:
                formula = '=IF({1}{0}="-", "-", 1 + MAX(\'Вода\'!$A$2:$A$100) + SUM(INDIRECT(ADDRESS(2,COLUMN({2}{0})) & ":" & ADDRESS(ROW(),COLUMN({2}{0})))))'\
                    .format(cur_row, COLUMNS['delimiter'].col_name, COLUMNS['delimiter_int'].col_name)

            colour = get_colour_by_name(v[COLUMNS['name']], skus)
            excel_client.colour = colour[1:]

            excel_client.draw_cell(row=cur_row, col=COLUMNS['boiling_number'].col, value=formula, set_border=False)
            excel_client.draw_row(row=cur_row, values=value, cols=column, set_border=False)
            if sku_is_rubber(skus, v[COLUMNS['name']]):
                excel_client.draw_cell(row=cur_row, col=COLUMNS['team_number'].col, value=2,
