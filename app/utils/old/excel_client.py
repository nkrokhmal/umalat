...
def generate_title(sheet):
    sheet.column_dimensions[
        openpyxl.utils.get_column_letter(COLUMNS["SKUS_ID"])
    ].hidden = True
    sheet.column_dimensions[
        openpyxl.utils.get_column_letter(COLUMNS["BOILING_ID"])
    ].hidden = True
...
def build_plan_sku(date, df, request_list, plan_path=None):
    ...
    request_list = sorted(
        ...
        key=lambda k: (
            k["IsLactose"],
            k["GroupSKU"][0]["SKU"].made_from_boilings[0].percent,
        ),
    )
    is_lactose = False
    for group_skus in request_list:
        if (
            group_skus["GroupSKU"][0]["SKU"].made_from_boilings[0].is_lactose
            != is_lactose
        ):
            cur_row += space_rows
        ...
        for group in current_app.config["ORDER"]:
            block_skus = [
                x for x in group_skus["GroupSKU"] if x["SKU"].group.name == group
            ]
            block_skus = sorted(
            ...
            for sku in block_skus:
                formula_plan = "=IFERROR(INDEX('{0}'!$A$5:$DK$265,MATCH($O$1,'{0}'!$A$5:$A$228,0),MATCH({1},'{0}'!$A$5:$DK$5,0)), 0)".format(
                    ...
                    block.sheet.cell(cur_row, CELLS["SKU"].column).coordinate,
                )
                ...
                block.draw_value(
                    row=cur_row, col=CELLS["Brand"].column, value=sku["SKU"].brand_name
                )
                block.draw_value(
                    row=cur_row, col=CELLS["SKU"].column, value=sku["SKU"].name
                )
                block.draw_value(
                ...
                block.draw_value(
                    ...
                    if sku["SKU"].production_by_request
                    else 0,
                )
                block.draw_value(
                    row=cur_row,
                ...
                group_formula.append(
                    block.sheet.cell(cur_row, CELLS["ProductionPlan"].column).coordinate
                )
                cur_row += 1
        ...
        block.merge_cells(
            ...
            value="{}% варка, {}{}".format(
                ...
                ", без лактозы"
                if not group_skus["GroupSKU"][0]["SKU"].made_from_boilings[0].is_lactose
                else "",
            ),
            alignment=Alignment(horizontal="center", vertical="center", wrapText=True),
        )
        block.draw_value(
            row=beg_row, col=CELLS["Volume"].column, value=group_skus["Volume"]
        )
        formula_boiling_count = "{}".format(
            str(group_formula).strip("[]").replace(",", " +").replace("'", "").upper()
        )
        block.draw_value(
            row=beg_row,
        ...
        block.draw_value(
            ...
            value="=ROUND({}, 0)".format(
                block.sheet.cell(beg_row, CELLS["Estimation"].column).coordinate.upper()
            ),
        )
        block.draw_value(
            row=beg_row,
        ...
        block.draw_value(
            row=beg_row, col=COLUMNS["BOILING_ID"], value=group_skus["BoilingId"]
        )
        if is_lactose:
            cur_row += space_rows
    ...
    for i in range(1, 200):
        if sheet_plan.cell(i, COLUMNS["BOILING_ID"]).value is not None:
            ...
            for j in range(i, 200):
                if sheet_plan.cell(j, CELLS["SKU"].column).value in [
                    x.name for x in sku_group
                ]:
                    sku_id = [
                        x.id
                        for x in sku_group
                        if x.name == sheet_plan.cell(j, CELLS["SKU"].column).value
                    ][0]
                    volume = abs(
                        excel.evaluate(
                            "'{}'!{}".format(
            ...
            if sheet_plan.cell(i, CELLS["BoilingVolumes"].column).value is not None:
                boiling_weights = re.split(
                    ", |. | ", sheet_plan.cell(i, CELLS["BoilingVolumes"].column).value
                )
                boiling_weights = [
                    x
                    for x in boiling_weights
                    if current_app.config["BOILING_VOLUME_LIMITS"]["MIN"]
                    <= int(x)
                    <= current_app.config["BOILING_VOLUME_LIMITS"]["MAX"]
                ]
                boiling_weights = [
                    int(x) for x in boiling_weights if isinstance(x, int) or x.isdigit()
                ]

            if len(boiling_weights) > boilings_count:
                boiling_weights = boilings_count * [
                    current_app.config["BOILING_VOLUME_LIMITS"]["MAX"]
                ]
            else:
                boiling_weights += int(boilings_count - len(boiling_weights)) * [
                    current_app.config["BOILING_VOLUME_LIMITS"]["MAX"]
                ]
