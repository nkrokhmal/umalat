...
def generate_constructor_df_v3(df_copy):
    ...
    df["boiling_type"] = df["boiling_id"].apply(
        lambda boiling_id: cast_boiling(boiling_id).boiling_type
    )
    ...
    for order in orders:
        ...
        if not order[0]:
            ...
            if total_sum // max_weight == total_sum_without_lactose // max_weight:
                if (df_filter_chl["plan"].sum() < 100) and (
                    df_filter_oth["plan"].sum() < 100
                ):
                    order = [df_filter_chl, df_filter_oth, df_filter_fdl]
                elif (df_filter_chl["plan"].sum() < 100) and (
                    df_filter_oth["plan"].sum() > 100
                ):
            ...
            df_filter_dict = df_filter.sort_values(
                by=["weight", "pack_weight"], ascending=[False, False]
            ).to_dict("records")
            boilings.add_group(df_filter_dict)
            ...
            for group, df_grouped in df_grouped_boiling_id.groupby("group"):
                ...
                if rubber_sku_exist:
                    ...
                    df_grouped_dict = df_grouped.sort_values(
                        by=["weight", "pack_weight"], ascending=[True, False]
                    ).to_dict("records")
...
def draw_constructor_template(df, file_name, wb, df_extra_packing):
    ...
    for value in sorted(form_factors, key=lambda x: x.weight_with_line):
        draw_row(
            boiling_form_factor_sheet, cur_i, [value.weight_with_line], font_size=8
        )
        cur_i += 1
        ...
        for v in values:
            if v[0] == "-":
                ...
                if sheet_name == "Вода":
                    v[
                        1
                    ] = '=IF(N{0}="-", "", 1 + SUM(INDIRECT(ADDRESS(2,COLUMN(Q{0})) & ":" & ADDRESS(ROW(),COLUMN(Q{0})))))'.format(
                        cur_i
                    )
                else:
                    v[
                        1
                    ] = '=IF(N{0}="-", "-", 1 + MAX(\'Вода\'!$A$2:$A$100) + SUM(INDIRECT(ADDRESS(2,COLUMN(Q{0})) & ":" & ADDRESS(ROW(),COLUMN(Q{0})))))'.format(
                        cur_i
                    )
