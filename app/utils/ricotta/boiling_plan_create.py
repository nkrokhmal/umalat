...
def boiling_plan_create(df):
    ...
    result = result[["id", "number_of_tanks", "group", "output", "name", "boiling_type", "kg"]]
    return result
...
def handle_ricotta(df):
    ...
    for order in orders:
        ...
        if not df_filter.empty:
            max_weight = df_filter[["number_of_tanks", "output_per_tank"]].prod(axis=1).iloc[0]
            boilings_ricotta.add_group(df_filter.to_dict('records'), max_weight=max_weight)
