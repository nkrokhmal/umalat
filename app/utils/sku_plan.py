...
class SkuPlanClient:
    def __init__(self, date, remainings, skus_grouped):
        ...
        self.filepath = os.path.join(current_app.config['SKU_PLAN_FOLDER'], self.filename)
        self.remainings = remainings
        self.skus_grouped = sorted(skus_grouped, key=lambda x: (x.boiling.is_lactose, x.boiling.percent))
        self.space_rows = 2
    ...
    def fill_remainigs_list(self):
        with pd.ExcelWriter(self.filepath, engine='openpyxl') as writer:
            ...
            self.remainings.to_excel(writer, sheet_name=current_app.config['SHEET_NAMES']['remainings'])
            writer.save()
        ...
        for sku_grouped in self.skus_grouped:
            ...
            for group_name in current_app.config['ORDER']:
                ...
                for block_sku in block_skus:
                    formula_plan = "=IFERROR(INDEX('{0}'!$A$5:$DK$265,MATCH($O$1,'{0}'!$A$5:$A$228,0),MATCH({1},'{0}'!$A$5:$DK$5,0)), 0)".format(
                        ...
                        excel_client.sheet.cell(cur_row, CELLS['SKU'].column).coordinate)
                    ...
                    excel_client.draw_cell(row=cur_row, col=CELLS['Brand'].column, value=block_sku.sku.brand_name)
                    excel_client.draw_cell(row=cur_row, col=CELLS['SKU'].column, value=block_sku.sku.name)
                    excel_client.draw_cell(row=cur_row, col=CELLS['FactRemains'].column, value=formula_plan)
                    excel_client.draw_cell(row=cur_row, col=CELLS['NormativeRemains'].column, value=formula_remains)
                    excel_client.draw_cell(
                        ...
                        value='=MIN({}, 0)'.format(
                            excel_client.sheet.cell(
                                cur_row,
                                CELLS['FactRemains'].column).coordinate) if block_sku.sku.production_by_request else 0)
                    excel_client.draw_cell(
                        row=cur_row,
                        col=CELLS['ExtraPacking'].column,
                        value=0 if block_sku.sku.packing_by_request else '=MIN({}, 0)'.format(
                            excel_client.sheet.cell(cur_row, CELLS['FactRemains'].column).coordinate))

                    formula_group.append(excel_client.sheet.cell(cur_row, CELLS['ProductionPlan'].column).coordinate)
                    cur_row += 1
            ...
            excel_client.draw_cell(
                ...
                value='=-({}) / {}'.format(
                    ...
                    excel_client.sheet.cell(beg_row, CELLS['Volume'].column).coordinate.upper()))

            excel_client.draw_cell(
                row=beg_row,
                col=CELLS['Plan'].column,
                value='=ROUND({}, 0)'.format(
                    excel_client.sheet.cell(beg_row, CELLS['Estimation'].column).coordinate.upper()))
