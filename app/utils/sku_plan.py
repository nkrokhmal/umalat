...
class SkuPlanClient:
    def __init__(self, date, remainings, skus_grouped, template_path, yesterday_boiling_plan_df=pd.DataFrame()):
        ...
        self.filepath = os.path.join(flask.current_app.config["SKU_PLAN_FOLDER"], self.filename)
    ...
    def fill_skus(
                    ...
                    excel_client.draw_cell(
                        ...
                        value=self._set_production_plan(
                            ...
                            "=MIN({} - {}, 0)".format(
                                ...
                                excel_client.sheet.cell(cur_row, CELLS["NotCalculatedRemainings"].column).coordinate,
                            ),
                        ),
                    )
                    excel_client.draw_cell(
        ...
        excel_client.draw_cell(row=beg_row, col=COLUMNS["BOILING_ID"], value=sku_grouped.boiling.id)
        if space_factor:
            cur_row += self.space_rows
        return cur_row

    def fill_mozzarella_sku_plan(self):
        self.skus_grouped = sorted(self.skus_grouped, key=lambda x: (x.boiling.is_lactose, x.boiling.percent))
    ...
    def fill_milk_project_sku_plan(self):
        ...
        for sku_grouped in self.skus_grouped:
            ...
            cur_row = self.fill_skus(
                ...
                cur_row,
                False,
                order=["Рикотта", "Качорикотта", "Четук", "Кавказский", "Черкесский", "Брынза", "Чанах"],
            )
            cur_row += self.space_rows
    ...
    def fill_adygea_sku_plan(self):
        ...
        for sku_grouped in self.skus_grouped:
            excel_client = ExcelBlock(sheet=sheet)
            cur_row = self.fill_skus(
                sku_grouped, excel_client, cur_row, False, order=["Кавказский", "Черкесский", "Рикотта"]
            )
            cur_row += self.space_rows
