...
class Line(db.Model):
    ...
    steam_consumption = db.relationship(
        "SteamConsumption", backref=backref("line", uselist=False, lazy="subquery")
    )
    ...
    @staticmethod
    def generate_lines():
        mozzarella_department = Department.query.filter_by(
            name="Моцарельный цех"
        ).first()
        for params in [
...
class CoolingTechnology(db.Model):
    ...
    form_factors = db.relationship(
        "FormFactor", backref=backref("default_cooling_technology", uselist=False)
    )
...
class FormFactor(db.Model):
    ...
    default_cooling_technology_id = db.Column(
        db.Integer, db.ForeignKey("cooling_technologies.id"), nullable=True
    )
    skus = db.relationship(
